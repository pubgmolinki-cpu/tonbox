...
async def handle(request):
    return web.Response(text="Симуляция активна")
...
Комментируй матч будто это реальный, каждые 6 минут, как человек: атаки, моменты, офсайды, угловые, VAR, жёлтые / красные, травмы (редко, с описанием и сроком по сезону), погода, эмоциональные всплески.

👥 ОБНОВЛЁННЫЕ ДАННЫЕ КО 2 ТАЙМУ

⚠️ ВАЖНО: все глобальные игровые механики, действовавшие в 1 тайме, ПРОДОЛЖАЮТ ДЕЙСТВОВАТЬ во 2 тайме: сезон, травмы, эмоции, мандраж. Ничего не обнуляется.

---

## 🔹 ВТОРОЙ ТАЙМ (46’–90’)

Продолжай матч, опираясь на: усталость из 1 тайма, тактики, замены, уже полученные травмы, эмоциональное состояние, давление трибун. Комментируй каждые 6 минут.

❗Важно: Описание атаки (начало → развитие → итог) обязательно для каждого момента.
