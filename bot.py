...
async def get_stats_api(request):
    """Отдает данные из базы в формате JSON для сайта"""
...
async def ask_ai_with_tools(user_text):
    prompt = f"""
    Ты менеджер TonScore. Если админ просит записать гол (н-р 'Месси забил 2'), ответь строго:
    ...
    if "[ACTION:ADD_GOAL" in ai_res:
        try:
            ...
            db_execute("INSERT INTO players (name, goals) VALUES (%s, %s) ON CONFLICT (name) DO UPDATE SET goals = players.goals + EXCLUDED.goals", (name, count))
            return f"✅ {name}: +{count} гол(а) в базе!"
...
async def cmd_start(message: Message):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="Открыть TonScore 🏆", web_app=WebAppInfo(url=f"https://{clean_url}"))]])
    await message.answer("<b>TonScore готов!</b>", parse_mode="HTML", reply_markup=kb)
