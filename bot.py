...
SIMULATION_FULL_TEMPLATE = """📝 ШАБЛОН СИМУЛЯЦИИ МАТЧА ПЕРВОГО ТАЙМА 

В этом чате ты комментируешь мои матчи.
...
## 🎙 СИМУЛЯЦИЯ МАТЧА (1-90 МИНУТ)

Опиши матч полностью согласно регламенту, включая 1-й и 2-й таймы, травмы, мандраж и эмоциональную шкалу. Комментируй каждые 6 минут.
...
async def start(m: types.Message):
    ...
    await m.answer("Привет, дорогой симер. Ты зашёл в бота для симуляции матчей.", reply_markup=kb.as_markup(resize_keyboard=True))
...
async def s1(m: types.Message, state: FSMContext):
    ...
    await m.answer("1️⃣ Введите название команды ХОЗЯЕВ:")
...
async def s2(m: types.Message, state: FSMContext):
    ...
    await m.answer("2️⃣ Введите название команды ГОСТЕЙ:")
...
async def s3(m: types.Message, state: FSMContext):
    ...
    await m.answer("3️⃣ Укажите стадион:")
...
async def s4(m: types.Message, state: FSMContext):
    ...
    await m.answer(f"4️⃣ Пришлите состав команды {d['h_n']}:")
...
async def s5(m: types.Message, state: FSMContext):
    ...
    d = await state.get_data()
    await m.answer(f"5️⃣ Пришлите состав команды {d['a_n']}:")
...
async def s6(m: types.Message, state: FSMContext):
    ...
    d = await state.get_data()
    await m.answer(f"6️⃣ Пришлите тактику команды {d['h_n']}:")
...
async def s7(m: types.Message, state: FSMContext):
    ...
    d = await state.get_data()
    await m.answer(f"7️⃣ Пришлите тактику команды {d['a_n']}:")
...
async def s_fin(m: types.Message, state: FSMContext):
    ...
    await m.answer("⏳ Симулирую по полному регламенту. Пожалуйста, подождите...")
    
    final_prompt = SIMULATION_FULL_TEMPLATE.format(match_input_data=data_block)
    res = await call_groq(final_prompt, "Ты футбольный комментатор. Комментируй строго по регламенту.")
    
    if len(res) > 4000:
        for i in range(0, len(res), 4000): await m.answer(res[i:i+4000])
    else: await m.answer(res)
    
    await log(f"ИТОГ МАТЧА ОТ ИИ:\n{res}", "ИИ Groq")
    await state.clear()
